export const convertirAInput =(botonOrigen:HTMLElement,IdFinal?:string,sessionStorageNombre:string='',tipo:string='text',volverADIV:boolean=false,funcionRecarga?:Function)=>{
    ...
    const idBotonOrigen = botonOrigen.id?botonOrigen.id:new Date().getTime().toString() // Almacena el id del div, y si no tiene lo genera aleatoriamente

    let valor:string
    if(sessionStorageNombre) valor = sessionStorage.getItem(sessionStorageNombre)?'':sessionStorage.getItem(sessionStorageNombre)! // Este es el valor que debe tener el input una vez 
    else valor = tituloBotonOrigen
